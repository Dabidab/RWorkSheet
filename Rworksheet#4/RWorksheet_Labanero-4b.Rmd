...
while(!is.numeric(start) || is.na(start)) {
...
pie(expenses, labels = paste(names(expenses), "(", round((expenses/sum(expenses))*100), "%)"), 
    col = c("black", "gray", "white", "orange"),
    main = "Monthly Expenses of Dela Cruz Family")

legend("topright", legend = names(expenses), fill = c("black", "gray", "white", "orange"))
...
```
``` {r num7}
mydata <- read_xlsx("alexa_file.xlsx")
mydata
```
